...
namespace RTHybridFastChemicalSynapse {

constexpr std::string_view MODULE_NAME = "RTHybrid FastChemical Synapse";

enum PARAMETER : Widgets::Variable::Id {
  // set parameter ids here
  ELECTRICAL_SYNAPSE_G = 0,
  ELECTRICAL_SYNAPSE_ESYN,
  ELECTRICAL_SYNAPSE_SFAST,
  ELECTRICAL_SYNAPSE_VFAST,
...
inline std::vector<Widgets::Variable::Info> get_default_vars() {
  return {
      ...
      {ELECTRICAL_SYNAPSE_VFAST, "Vfast", "",
       Widgets::Variable::DOUBLE_PARAMETER, -1.66},
...
private:
  double period;
  double i, offset, scale, Esyn, Sfast, Vfast;
  double g[1];
  ...
  ;
  void sm_fast_chemical(double v_post, double v_pre, double *ret);
};
